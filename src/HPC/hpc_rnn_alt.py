"""
用gate-rnn的方式来实现HPC的Alternation任务
但是加上了EC5的直连。
24.08.07
...
class HPCrnn_alt(nn.Module):
    def __init__(self, ts=0.1, ecnum=100, ca1num=100, ca3num=100, ca3sigma=5, tracklength=100, actnum=2):
        ...
        """
        super(HPCrnn_alt, self).__init__()
        self.ts = ts  # 时间步长
        self.ecnum = ecnum
        self.ca1num = ca1num
        self.actnum = actnum
        ...
        self.wec5dir = nn.Parameter(torch.rand(ca1num, ecnum) * 0.1)
        self.ec5dirbias = nn.Parameter(torch.zeros(ecnum))
        ...
        nn.init.xavier_normal_(self.wec5dir)
    ...
    def forward(self, cue, ec3_last, ec5_last, ca1_last):
        ...
        for x in range(self.tracklength):
            ...
            # ec5 = 0.69 + 0.3 * torch.sigmoid(4 * (ec5 - 0.3))  # 与y=x的交点为0.97，最大值0.99，比较符合生物
            ec5 = 0.70 + 0.3 * torch.sigmoid(4 * (ec5 - 0.3))  # 与y=x的交点为0.97，最大值1，相当于可以完全记忆

            ec5dir = torch.relu(torch.matmul(ca1, self.wec5dir) + self.ec5dirbias)

            ec3 = ec5 * ec3  # EC3根据对应的EC5发放率进行衰减
            ec3 = ec3 + 0.1 * ec5dir  # 加上ec5“直驱”细胞的输入
...
def net_train(bs=30, lr=0.003):
    ...
    ecnum = 100
    actnum = 10
    cue_ratio = 0.1  # 每个线索激活多少比例的EC3细胞，如果按照Magee来就是0.12
    net = HPCrnn_alt(ca1num=ca1num, ecnum=ecnum, tracklength=tracklength, actnum=actnum)
